...
export default function OAuthAuthorizePage() {
  ...
  useEffect(() => {
    // Check if user is logged in to Adda
    const token = localStorage.getItem('adda_token');
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3100/api/oauth/authorize/approve', {
        method: 'POST',
        headers: {
      ...
      <div className="max-w-md w-full space-y-8 p-10 bg-white rounded-xl shadow-lg">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            <strong className="text-teal-600">{clientName}</strong> wants to access your Adda account
          </p>
        </div>
        ...
        <div className="space-y-3">
          <button
            ...
            disabled={loading}
            className="w-full py-3 px-4 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-lg shadow-md transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
